...
class LinAngularXCA_CA(nn.Module):
    def __init__(self, in_channels):
        ...
        self.la = LinAngularAttention(in_channels=16)
        self.xa = XCA(dim=16)
    ...
    def forward(self, x1, x2):
        ...
        result = to_4d(result, h, w)

        return result


class LinAngularXCA_CA_1(nn.Module):
    def __init__(self, in_channels):
        super(LinAngularXCA_CA_1, self).__init__()
        self.la = LinAngularAttention(in_channels=in_channels)
        self.xa = XCA(dim=in_channels)
        self.cafm = CAFM(in_channels)
...
if __name__ == '__main__':
    ...
    input1 = torch.randn(4, 32, 128, 128)
    output = block(input1, input1)  # 获取两个输入的输出
