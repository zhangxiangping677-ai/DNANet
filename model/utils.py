...
def save_model(mean_IOU, best_iou, save_dir, save_prefix, train_loss, test_loss, recall, precision, epoch, net):
    if mean_IOU > best_iou:
        ...
        save_ckpt({
            ...
            filename='mIoU_' + '_' + save_prefix + epoch + '.pth.tar')
...
def save_Pred_GT(pred, labels, target_image_path, val_img_ids, num, suffix):
    ...
    if val_img_ids[num] in {"000107", "000380", "000523", "000717"}:
        # img_tensor = torch.cat([pred[0], pred[1], pred[2], pred[3]], dim=1)
        freq_fun(None, pred, '../result/fre_res_2/' + val_img_ids[num] + '.png')


def freq_fun(self, all_feat_layer, savename):
    """
    该函数通过对输入的特征图（all_feat_layer）应用二维傅里叶变换，将其转换到频域。
    然后，它可视化频域信息并保存输出图像。

    参数：
    all_feat_layer (Tensor): 特征图，形状应为 (B, C, H, W)，其中
                              B 是批大小，C 是通道数，H 和 W 是特征图的高度和宽度。
    savename (str): 处理后的频域图像保存路径。
    """

    # 提取批大小、通道数、高度和宽度
    B, C, H, W = all_feat_layer.shape
    print(f"特征图的形状: {all_feat_layer.shape}")

    # 将特征图从 PyTorch 张量转换为 NumPy 数组
    all_feat_layer = all_feat_layer.cpu().detach().numpy()
